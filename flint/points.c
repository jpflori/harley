...
/* Let's use higher level functions */
void padic_poly_teichmuller(padic_poly_t rop, const padic_poly_t op,
                            padic_ctx_t pctx)
{
    if (pctx->N == 1)
    {
    ...
    {
        long *a, d, i, j, l, l0, l1, n;
        padic_ctx_struct *ctx;
        padic_poly_t f0, f0_sqr, f1, f1_sqr, V, delta;

        /* Degree and length */
        d = padic_poly_degree(op);
        l = d + 1;
        l1 = (l >> 1);
        l0 = l1 + (l % 2);

        /* Number of steps */
        n = FLINT_CLOG2(pctx->N) + 1;

        /* Precision at each step */
        a = flint_malloc(n * sizeof(long));
        ctx = flint_malloc(n * sizeof(padic_ctx_struct));
        i = 0;
        {
            a[i] = pctx->N;
            ctx = pctx;
        ...
        for (i++; a[i - 1] > 1; i++) {
            a[i] = (a[i - 1] + 1) / 2;
            padic_ctx_init(ctx + i, pctx->p, a[i], pctx->mode);
        }
        padic_poly_init2(f0, l0);
        padic_poly_init2(f0_sqr, l);
        padic_poly_init2(f1, l1);
        padic_poly_init2(f1_sqr, l);
        padic_poly_init2(V, l);
        padic_poly_init2(delta, l);
        ...
        for (i--; i >= 0; i--)
        {
            /* f_0 and f_1 */
            for (j = 0; j < l; j++)
                    fmpz_set(((j % 2)?f1:f0)->coeffs + (j >> 1), rop->coeffs + j);
            /* ... */

            padic_poly_pow(f0_sqr, f0, 2, ctx + i);
            padic_poly_pow(f0_sqr, f0, 2, ctx + i);

            /* V */
            for (j = 0; j < l; j++)
                fmpz_set(V->coeffs + i, rop->coeffs + i);
        }
